...
鲶鱼精邮差插件本身接收 JSON 文本指令标点，以便其他程序直接通过代码生成对象并转化成对应的指令；

而 Triggernometry 触发器的输入则是用户手动拼接的文本，拼凑 JSON 字符串的难度较高。

因此，本工具的目的是从 Triggernometry 接收一串人类更加易懂、易输入的文本，将其处理成鲶鱼精邮差预期的 JSON 指令，并转化为标点。

用户无需手动输入 JSON 字符串，或构建标点数据结构并序列化，显著简化开发 Triggernometry 触发器时调用场地标点的操作。
