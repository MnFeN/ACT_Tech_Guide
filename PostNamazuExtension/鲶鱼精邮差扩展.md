...
| 设置模型状态 | `SetModelStatus` | `[address], [value]` | `[value]`: <br />Visible（显示）; <br />Unloaded（未加载）;<br />Hidden（隐藏） | `${_me.address}, Hidden` |
...
| 设置模型透明度 | `SetOpacity` | `[address], [value]` |  | `${_me.address}, 5` |
| 重绘模型 | `Redraw` | `[address]` |  | `${_me.address}` |
| 设置实体边框颜色 | `SetHighlightColor` | `[address], [value]` | `[value]`: 0-7 | `${_me.address}, 5` |
| 本地移除状态 | `RemoveStatus` | `[address], [statusId]` | `[statusId]`: 要移除的状态（buff） ID | `${_me.address}, 3` |
...
- 键值对形式：手动指定要设置的参数
  ...
  - `AngleH` 当前水平视角
  - `AngleV` 当前垂直视角，上 → 下为 -π/2 → π/2
  - `MinAngleV` 最小垂直视角，原始值 -1.484（仰视 85°），默认配置 -1.569 （仰视 89.9°）
  - `MaxAngleV` 最大垂直视角，原始值  0.785（俯视 45°），默认配置  1.569 （俯视 89.9°）
