...
- **id** / **name**
  ...
  但 7.0 版本后，网络包中不再包含标点者的信息。考虑到格式兼容性，这两个字段被固定设为自身的 ID 与名称。
...
- **id** / **name**
  ...
  若已存在某标点时，对其他人使用此标点，导致标点被 “顶掉”，对应的 Delete 日志中 `id` 为 `0000`，`name` 为空（如下）。
...
[01:04:52.452] SignMarker 1D:Delete:2:0000::10FF0001:Tini Poutini
```
...
参见：[nari director update 文档](https://xivlogs.github.io/nari/types/director.html)

为了控制用户界面的各个方面，游戏会发送一系列数据包（Actor Controls）。
这些数据包分为 3 种类型：ActorControl、ActorControlSelf 和 ActorControlTarget。
ActorControl 是全局的，ActorControlSelf / ActorControlTarget 会影响单个或多个特定角色。
DirectorUpdate 是 ActorControlSelf 的一个类别，用于控制特定玩家在副本中的事件，如：
...
已知的类型：
  ...
  |--------------------------------|-----------------------------------------------------------|-------|
  | 初次进入副本实例                | `21:content:40000001:time:`                               | time is the lockout time in seconds |
  | 副本胜利（多变迷宫）            | `21:content:40000002:00:00:00:00`                         |       |
  | 副本胜利                        | `21:content:40000003:00:00:00:00`                        |       |
  | 团灭黑屏                       | `21:content:40000005:00:00:00:00`                         | 团灭最早出现的日志 |
  | “重新挑战！”                    | `21:content:40000006:time:00:00:00`                       | 团灭后出现此行文字提示时的日志 |
  | 未知，但频繁出现                | `21:content:40000007:value1:value2:00:00`                 |       |
  | 团灭日志1                       | `21:content:40000013:00:00:00:00`                         | 团灭日志 1-3 间隔很短，在黑屏后按 1-3 顺序出现。<br />极个别早期副本没有这些日志，如带有区域封锁的巴哈姆特大迷宫副本。 |
  | 团灭日志2                       | `21:content:4000000F:time:00:00:00`                       |       |
  | 团灭日志3                       | `21:content:40000011:00:00:00:00`                         | 这条日志出现时意味着这场战斗的 debuff、实体等已经全部移除。 |
  | BGM 切换                        | `21:content:80000001:音乐id:00:00:00`                      |       |
  | 调整副本剩余时间                 | `21:content:80000004:time:00:00:00`                       |       |
  | Boss 极限技（进度条）充能        | `21:content:8000000C:value1:value2:00:00`                 |       |
  | Boss 台词                       | `21:content:80000027:台词id:value2:00:00`                 |       |
...
- **更新：**
  ...
  对游戏代码逆向分析后发现，以上参数实际应为 `flag1 flag2:index`。

  如 `00020001:08` 代表 `flag1 = 0x0002`, `flag2 = 0x0001`, `index = 0x08`。

  游戏内存中存储了当前所有 MapEffect 的长度为 128 的数组，其中 `index` 对应数组索引 `0 - 127`，可以大致理解成 “什么位置的特效”。

  `flag` (ushort) 的每一个二进制位代表一种特效的开关，相当于有 16 个通道，可以理解为 “这个位置出现哪种特效”。

  目前似乎只实际使用了 `flag1`，而 `flag2` 被忽略。

  对于 `flag1`，目前使用过的值均只有一个二进制位为 1（如 0x2, 0x40, 0x100），代表此 index 的位置只开启这一个通道的特效、关闭其他 15 个通道的特效（如果有的话）。
...
此日志行与[危命任务](#line257)类似，在博兹雅等地图的遭遇战 (CE，含岛内的大型副本) 生成、移除、进度变化时生成。
...
- **`popTime`**
  ...
  十六进制遭遇战开始时间（Unix 时间戳）。

  如：`68A1931D`，即十进制的 `1755419421`，代表 UTC 时间 `2025-08-17 08:30:21`（北京时间 `2025-08-17 16:30:21`）

  注：触发器可使用 `${_systemtime}` 获取当前时间戳。

- **`timeRemaining`**
  
  十六进制遭遇战剩余时间。

  遭遇战开始前、结束后为 `0000`，开始后从初始值（如 `04B0` = `1200`，即 20 分钟）逐渐下降。
...
- **`progress`**
  ...
  单一 Boss 类型的战斗通常从 `00` 逐渐增加至 `63`；多实体/多阶段的战斗可能仅会在一个阶段结束后跳跃性增加。

遭遇战通常由一条 `status` = `01` 的日志起始，倒计时结束后若范围内有玩家，则生成 `status` = `02` 的日志，随后在战斗中持续生成 `status` = `03` 的日志，最终结束后生成 `status` = `00` 的日志。

若倒计时结束后范围内没有玩家，则遭遇战直接结束，生成 `status` = `00` 的日志。
